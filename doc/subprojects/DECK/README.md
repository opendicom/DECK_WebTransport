...
Between converter and pacs an exam sqlite script file can be created on one side 
and read on the other.

Between pacs and browser, a simple serialization based on UDP datagrams copies 
the attributes from the server to the client, which writes them into its database.
