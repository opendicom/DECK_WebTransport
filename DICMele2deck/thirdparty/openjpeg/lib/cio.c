        ...
        OPJ_BOOL l_is_input)
{
    ...
    l_stream = (opj_stream_private_t*) calloc(1, sizeof(opj_stream_private_t));
    if (! l_stream) {
        return 00;
    }

    l_stream->m_buffer_size = p_buffer_size;
    l_stream->m_stored_data = (OPJ_BYTE *) malloc(p_buffer_size);
                                 ...
                                 OPJ_SIZE_T p_size)
{
    ...
    for (;;) {
        ...
        if (! opj_stream_flush(p_stream)) return (OPJ_SIZE_T) - 1;
    }
}
                                ...
                                OPJ_OFF_T p_size, opj_event_mgr_t * p_event_mgr)
{
    ...
    l_is_written = opj_stream_flush(p_stream);
    if (! l_is_written) {
        p_stream->m_status |= OPJ_STREAM_STATUS_ERROR;
        p_stream->m_bytes_in_buffer = 0;
        return (OPJ_OFF_T) - 1;
