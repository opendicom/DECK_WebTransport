...
opj_tcd_t* opj_tcd_create(OPJ_BOOL p_is_decoder)
{
    ...
    l_tcd = (opj_tcd_t*) calloc(1, sizeof(opj_tcd_t));
    if (!l_tcd) {
        return 00;
    }
    ...
    l_tcd->tcd_image = (opj_tcd_image_t*)calloc(1, sizeof(opj_tcd_image_t));
    if (!l_tcd->tcd_image) {
        free(l_tcd);
        return 00;
    }
                              ...
                              opj_codestream_info_t *cstr_info)
{
    ...
    if (cstr_info) {
        ...
        tile_info->thresh = (OPJ_FLOAT64 *) malloc(tcd_tcp->numlayers * sizeof(
                                OPJ_FLOAT64));
        if (!tile_info->thresh) {
                      ...
                      opj_thread_pool_t* p_tp)
{
    ...
    p_tcd->tcd_image->tiles = (opj_tcd_tile_t *) calloc(1,
                              sizeof(opj_tcd_tile_t));
    if (! p_tcd->tcd_image->tiles) {
        return OPJ_FALSE;
    }

    p_tcd->tcd_image->tiles->comps = (opj_tcd_tilecomp_t *) calloc(
        ...
        opj_event_mgr_t* manager)
{
    ...
    for (compno = 0; compno < l_tile->numcomps; ++compno) {
        ...
        if (l_tilec->resolutions == 00) {
            l_tilec->resolutions = (opj_tcd_resolution_t *) malloc(l_data_size);
            ...
            opj_tcd_resolution_t* new_resolutions = (opj_tcd_resolution_t *) realloc(
        ...
        for (resno = 0; resno < l_tilec->numresolutions; ++resno) {
            ...
            for (bandno = 0; bandno < l_res->numbands; ++bandno, ++l_band, ++l_step_size) {
                ...
                if (!l_band->precincts && (l_nb_precincts > 0U)) {
                    l_band->precincts = (opj_tcd_precinct_t *) malloc(/*3 * */
                                            l_nb_precinct_size);
                    if (! l_band->precincts) {
                    ...
                    opj_tcd_precinct_t * new_precincts = (opj_tcd_precinct_t *) realloc(
                ...
                for (precno = 0; precno < l_nb_precincts; ++precno) {
                    ...
                    if (!l_current_precinct->cblks.blocks && (l_nb_code_blocks > 0U)) {
                        l_current_precinct->cblks.blocks = malloc(l_nb_code_blocks_size);
                        ...
                        void *new_blocks = realloc(l_current_precinct->cblks.blocks,
                                                       l_nb_code_blocks_size);
                        if (! new_blocks) {
        ...
        p_code_block)
{
    if (! p_code_block->layers) {
        ...
        p_code_block->layers = (opj_tcd_layer_t*) calloc(100,
    ...
    if (! p_code_block->passes) {
        p_code_block->passes = (opj_tcd_pass_t*) calloc(100,
        ...
        p_code_block)
{
    ...
    if (l_data_size > p_code_block->data_size) {
        if (p_code_block->data) {
            ...
            free(p_code_block->data - 1);
        }
        p_code_block->data = (OPJ_BYTE*) malloc(l_data_size + 1);
        if (! p_code_block->data) {
        ...
        p_code_block)
{
    if (! p_code_block->segs) {
        ...
        p_code_block->segs = (opj_tcd_seg_t *) calloc(OPJ_J2K_DEFAULT_NB_SEGS,
                             ...
                             opj_tcd_marker_info_t* p_marker_info)
{
    ...
    if (p_tcd->cur_tp_num == 0) {
        ...
        if (p_cstr_info)  {
            ...
            p_cstr_info->tile[p_tile_no].packet = (opj_packet_info_t*) calloc((
                            ...
                            )
{
    ...
    free(p_tcd->used_component);
    p_tcd->used_component = NULL;

    if (numcomps_to_decode) {
        OPJ_BOOL* used_component = (OPJ_BOOL*) calloc(sizeof(OPJ_BOOL),
    ...
    if (p_cstr_info) {
        ...
        p_cstr_info->tile[p_tile_no].packet = (opj_packet_info_t *) malloc(
...
static OPJ_BOOL opj_tcd_mct_decode(opj_tcd_t *p_tcd, opj_event_mgr_t *p_manager)
{
    ...
    if (l_tile->numcomps >= 3) {
        if (l_tcp->mct == 2) {
            ...
            l_data = (OPJ_BYTE **) malloc(l_tile->numcomps * sizeof(OPJ_BYTE*));
            if (! l_data) {
                return OPJ_FALSE;
            }
...
opj_tcd_marker_info_t* opj_tcd_marker_info_create(OPJ_BOOL need_PLT)
{
    opj_tcd_marker_info_t *l_tcd_marker_info =
        (opj_tcd_marker_info_t*) calloc(1, sizeof(opj_tcd_marker_info_t));
...
void opj_tcd_marker_info_destroy(opj_tcd_marker_info_t *p_tcd_marker_info)
{
    if (p_tcd_marker_info) {
        free(p_tcd_marker_info->p_packet_size);
        free(p_tcd_marker_info);
    }
}
