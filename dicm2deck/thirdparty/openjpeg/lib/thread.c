...
{
    opj_mutex_t* mutex = (opj_mutex_t*) calloc(1U, sizeof(opj_mutex_t));
    if (mutex != NULL) {
...
{
    opj_cond_t* cond = (opj_cond_t*) malloc(sizeof(opj_cond_t));
    if (!cond) {
        return NULL;
    }
...
opj_thread_t* opj_thread_create(opj_thread_fn thread_fn, void* user_data)
{
    ...
    thread = (opj_thread_t*) malloc(sizeof(opj_thread_t));
    if (!thread) {
        return NULL;
    }
...
{
    return (opj_tls_t*) calloc(1, sizeof(opj_tls_t));
}
...
static void opj_tls_destroy(opj_tls_t* tls)
{
    ...
    for (i = 0; i < tls->key_val_count; i++) {
        if (tls->key_val[i].free_func) {
            tls->key_val[i].free_func(tls->key_val[i].value);
        }
    }
    free(tls->key_val);
    free(tls);
                     ...
                     opj_tls_free_func free_func)
{
    ...
    for (i = 0; i < tls->key_val_count; i++) {
        if (tls->key_val[i].key == key) {
            ...
            tls->key_val[i].free_func = free_func;
            return OPJ_TRUE;
        }
    }
    new_key_val = (opj_tls_key_val_t*) realloc(tls->key_val,
    ...
    new_key_val[tls->key_val_count].free_func = free_func;
    tls->key_val_count ++;
    return OPJ_TRUE;
}
...
opj_thread_pool_t* opj_thread_pool_create(int num_threads)
{
    ...
    tp = (opj_thread_pool_t*) calloc(1, sizeof(opj_thread_pool_t));
    if (!tp) {
        return NULL;
    }
    tp->state = OPJWTS_OK;
...
static OPJ_BOOL opj_thread_pool_setup(opj_thread_pool_t* tp, int num_threads)
{
    ...
    tp->worker_threads = (opj_worker_thread_t*) calloc((size_t)num_threads,
    ...
    OPJ_BOOL signal_job_finished)
{
    while (OPJ_TRUE) {
        ...
        if (!worker_thread->marked_as_waiting) {
            ...
            item = (opj_worker_thread_list_t*) malloc(sizeof(opj_worker_thread_list_t));
            if (item == NULL) {
                                    ...
                                    void* user_data)
{
    ...
    job = (opj_worker_thread_job_t*)malloc(sizeof(opj_worker_thread_job_t));
    if (job == NULL) {
        return OPJ_FALSE;
    }
    job->job_fn = job_fn;
    ...
    item = (opj_job_list_t*) malloc(sizeof(opj_job_list_t));
    if (item == NULL) {
        free(job);
        return OPJ_FALSE;
    }
    ...
    if (tp->waiting_worker_thread_list) {
        ...
        opj_worker_thread_list_t* to_free;
        ...
        to_free = tp->waiting_worker_thread_list;
...
void opj_thread_pool_destroy(opj_thread_pool_t* tp)
{
    ...
    if (tp->cond) {
        ...
        while (tp->waiting_worker_thread_list != NULL) {
            ...
            free(tp->waiting_worker_thread_list);
            tp->waiting_worker_thread_list = next;
        }
